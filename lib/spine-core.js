...
var spine;
(function (spine) {
    var UnityAtlas = (function () {
        function UnityAtlas(input) {
            this.atlasMap = {};
            this.file_name = '';
            var atlasData = input;
            this.atlasList = [];
            for (var i = 0; i < atlasData.mSprites.length; ++i) {
                this.atlasList[i] = UnityAtlasItem.fromJSON(atlasData.mSprites[i]);
                this.atlasMap[this.atlasList[i].name] = i;
            }
        }
        UnityAtlas.prototype.getMesh = function (name, width, height, start_x, start_y, scale_x, scale_y, flip_x, flip_y) {
            if (width === void 0) { width = -1; }
            if (height === void 0) { height = -1; }
            if (start_x === void 0) { start_x = 0; }
            if (start_y === void 0) { start_y = 0; }
            if (scale_x === void 0) { scale_x = 1; }
            if (scale_y === void 0) { scale_y = 1; }
            if (flip_x === void 0) { flip_x = false; }
            if (flip_y === void 0) { flip_y = true; }
            if (this.atlasMap[name] || this.atlasMap[name] == 0) {
                var result = UnityAtlasItem.fromJSON(this.atlasList[this.atlasMap[name]]).
                    getMesh(width, height, start_x, start_y, scale_x, scale_y, flip_x, flip_y);
                ...
                if (this.file_name.trim() != '')
                    file_message += " in '" + this.file_name + "'";
                throw new Error("Atlas '" + name + "' not found" + file_message + "!");
            }
        };
        UnityAtlas.load = function (loader, json_path, texture_path, success, error) {
            if (success === void 0) { success = null; }
            if (error === void 0) { error = null; }
            loader.loadText(json_path, function (path, atlasData) {
                loader.loadTexture(texture_path, function (imagePath, _) {
                    try {
                        var result = new UnityAtlas(JSON.parse(atlasData));
                        result.texture = loader.get(texture_path);
                        result.file_name = json_path;
                        if (success)
                            success(json_path, texture_path, result);
                    }
                    catch (exception) {
                        if (error)
                            error(json_path, texture_path, "Couldn't load texture " + imagePath);
                    }
                }, function (imagePath, errorMessage) {
                    if (error)
                        error(json_path, texture_path, "Couldn't load texture " + imagePath);
                });
            }, function (path, responseText) {
                if (error)
                    error(json_path, texture_path, "Couldn't load js atlas " + json_path + ": status " + status + ", " + responseText);
            });
        };
        return UnityAtlas;
    }());
    spine.UnityAtlas = UnityAtlas;
    var UnityFileData = (function () {
        function UnityFileData() {
        }
        return UnityFileData;
    }());
    spine.UnityFileData = UnityFileData;
    var UnityAtlasItem = (function () {
        function UnityAtlasItem() {
        }
        UnityAtlasItem.fromJSON = function (input) {
        ...
        UnityAtlasItem.prototype.getMesh = function (input_width, input_height, _start_x, _start_y, _scale_x, _scale_y, _flip_x, _flip_y) {
            if (input_width === void 0) { input_width = -1; }
            if (input_height === void 0) { input_height = -1; }
            if (_start_x === void 0) { _start_x = 0; }
            if (_start_y === void 0) { _start_y = 0; }
            if (_scale_x === void 0) { _scale_x = 1; }
            if (_scale_y === void 0) { _scale_y = 1; }
            if (_flip_x === void 0) { _flip_x = false; }
            if (_flip_y === void 0) { _flip_y = true; }
            var _width = input_width <= 0 ? this.width : input_width;
            var _height = input_height <= 0 ? this.height : input_height;
            var x_data = [
            ...
            var border_width = this.borderLeft + this.paddingLeft + this.borderRight + this.paddingRight;
            var middle_width = this.width - border_width;
            var _middle_width = _width - border_width;
            var middle_count = Math.floor(_middle_width / middle_width);
            if (_middle_width > 0 && middle_count > 0) {
                x_data = [];
                var _middle_item_width = _middle_width / middle_count;
                var iterator_mesh = 0;
                var iterator_tex = this.x;
                if (this.paddingLeft > 0)
                    x_data.push(new TexturePos(new PointPos(iterator_mesh, 0), new PointPos(iterator_tex, 0)), new TexturePos(new PointPos(iterator_mesh += this.paddingLeft, 0), new PointPos(iterator_tex += this.paddingLeft, 0)));
                if (this.borderLeft > 0)
                    x_data.push(new TexturePos(new PointPos(iterator_mesh, 0), new PointPos(iterator_tex, 0)), new TexturePos(new PointPos(iterator_mesh += this.borderLeft, 0), new PointPos(iterator_tex += this.borderLeft, 0)));
                for (var i = 0; i < middle_count; ++i) {
                    x_data.push(new TexturePos(new PointPos(iterator_mesh, 0), new PointPos(iterator_tex, 0)), new TexturePos(new PointPos(iterator_mesh += _middle_item_width, 0), new PointPos(iterator_tex + middle_width, 0)));
                }
                iterator_tex += middle_width;
                if (this.borderRight > 0)
                    x_data.push(new TexturePos(new PointPos(iterator_mesh, 0), new PointPos(iterator_tex, 0)), new TexturePos(new PointPos(iterator_mesh += this.borderRight, 0), new PointPos(iterator_tex += this.borderRight, 0)));
                if (this.paddingRight > 0)
                    x_data.push(new TexturePos(new PointPos(iterator_mesh, 0), new PointPos(iterator_tex, 0)), new TexturePos(new PointPos(iterator_mesh += this.paddingRight, 0), new PointPos(iterator_tex += this.paddingRight, 0)));
            }
            var border_height = this.borderTop + this.paddingTop + this.borderBottom + this.paddingBottom;
            var middle_height = this.height - border_height;
            var _middle_height = _height - border_height;
            var middle_y_count = Math.floor(_middle_height / middle_height);
            if (_middle_height > 0 && middle_y_count > 0) {
                y_data = [];
                var _middle_item_height = _middle_height / middle_y_count;
                var iterator_mesh = 0;
                var iterator_tex = this.y;
                if (this.paddingTop > 0)
                    y_data.push(new TexturePos(new PointPos(0, iterator_mesh), new PointPos(0, iterator_tex)), new TexturePos(new PointPos(0, iterator_mesh += this.paddingTop), new PointPos(0, iterator_tex += this.paddingTop)));
                if (this.borderTop > 0)
                    y_data.push(new TexturePos(new PointPos(0, iterator_mesh), new PointPos(0, iterator_tex)), new TexturePos(new PointPos(0, iterator_mesh += this.borderTop), new PointPos(0, iterator_tex += this.borderTop)));
                for (var i = 0; i < middle_y_count; ++i) {
                    y_data.push(new TexturePos(new PointPos(0, iterator_mesh), new PointPos(0, iterator_tex)), new TexturePos(new PointPos(0, iterator_mesh += _middle_item_height), new PointPos(0, iterator_tex + middle_height)));
                }
                iterator_tex += middle_height;
                if (this.borderBottom > 0)
                    y_data.push(new TexturePos(new PointPos(0, iterator_mesh), new PointPos(0, iterator_tex)), new TexturePos(new PointPos(0, iterator_mesh += this.borderBottom), new PointPos(0, iterator_tex += this.borderBottom)));
                if (this.paddingBottom > 0)
                    y_data.push(new TexturePos(new PointPos(0, iterator_mesh), new PointPos(0, iterator_tex)), new TexturePos(new PointPos(0, iterator_mesh += this.paddingBottom), new PointPos(0, iterator_tex += this.paddingBottom)));
            }
            var result_data = new AtlasTextureMesh();
            result_data.vertex = [];
            for (var _y = 0; _y < y_data.length; ++_y) {
                for (var _x = 0; _x < x_data.length; ++_x) {
                    var result_x = _flip_x ? this.width - x_data[_x].meshPosition.x : x_data[_x].meshPosition.x;
                    var result_y = _flip_y ? this.height - y_data[_y].meshPosition.y : y_data[_y].meshPosition.y;
                    result_data.vertex.push(new TexturePos(new PointPos(_start_x + result_x * _scale_x, _start_y + result_y * _scale_y), new PointPos(x_data[_x].texturePosition.x, y_data[_y].texturePosition.y)));
                }
            }
            result_data.index = [];
            for (var _y = 0; _y < y_data.length / 2; ++_y) {
                for (var _x = 0; _x < x_data.length / 2; ++_x) {
                    var index = _y * x_data.length * 2 + _x * 2;
                    var index2 = index + x_data.length;
                    result_data.index.push(index, index2 + 1, index + 1, index, index2, index2 + 1);
                }
            }
        ...
        return UnityAtlasItem;
    }());
    spine.UnityAtlasItem = UnityAtlasItem;
    ...
    var TexturePos = (function () {
        function TexturePos(meshPosition, texturePosition) {
            this.meshPosition = meshPosition;
            this.texturePosition = texturePosition;
        }
        return TexturePos;
    }());
    var AtlasTextureMesh = (function () {
        function AtlasTextureMesh() {
        }
        return AtlasTextureMesh;
    }());
    spine.AtlasTextureMesh = AtlasTextureMesh;
    var UnityAtlasData = (function () {
        function UnityAtlasData() {
        }
        return UnityAtlasData;
    }());
    spine.UnityAtlasData = UnityAtlasData;
})(spine || (spine = {}));
var spine;
(function (spine) {
